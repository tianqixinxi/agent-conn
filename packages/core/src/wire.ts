...
export const wireRoutes = {
  ...
  /** All `/ch/:channel` path parameters are opaque channelIds, never human aliases. */
...
export const PostJoinRespSchema = z.object({
  /** Opaque route identity. Kept in the legacy `channel` field for wire compatibility. */
  channel: ChannelNameSchema,
  channelId: ChannelNameSchema.optional(),
  /** Human channel alias; unlike `channel`, this may repeat across channels. */
...
export const PostCreateChannelReqSchema = z.object({
  ...
  /** Human channel alias. `displayName` remains a separate presentation label. */
  name: ChannelNameSchema.optional(),
