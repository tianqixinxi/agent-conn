 ...
 * - Channel.name 是人类别名，Channel.channelId 是路由身份；Channel.home = 'local:<hub 文件绝对路径>' 或 relay 的 https URL
...
/** Opaque route identity. It is deliberately separate from the human channel name. */
export const ChannelIdSchema = z.string().regex(NAME_RE)
export const MessageIdSchema = z.string().min(3).max(128)
export const NodeIdSchema = z.string().min(3).max(128)
export const RuntimeInstanceIdSchema = z.string().regex(/^r-[a-zA-Z0-9_-]{8,128}$/)
...
export const ChannelSchema = z.object({
  /** Human-readable alias; it is not required to be unique. */
  name: ChannelNameSchema,
  /** Stable opaque route identity. Optional only for pre-channelId local records. */
...
export const MessageEnvelopeSchema = z.object({
  ...
  /** Signed source harness run; profile/node identity remains the authoritative sender. */
...
export const DeliveryHoldDecisionSchema = z.object({
  messageId: MessageIdSchema,
  channelId: ChannelIdSchema,
  decision: z.enum(['deliver', 'drop', 'edit']),
  editedPayload: z.unknown().optional(),
  editedContentType: z.string().optional(),
  decidedAt: IsoSchema,
  decidedBy: z.literal('human'),
})
export type DeliveryHoldDecision = z.infer<typeof DeliveryHoldDecisionSchema>

export const AuthorizationReceiptSchema = z.object({
  authorizationId: z.string().regex(/^auth-[a-zA-Z0-9_-]{8,128}$/),
  taskId: z.string().min(1),
  scope: z.unknown(),
  decision: z.enum(['approve', 'reject']),
  decidedAt: IsoSchema,
  source: z.enum(['local-host', 'oauth', 'passkey', 'external-approval-service']),
  assurance: z.enum(['host-reported', 'cryptographically-verified']),
...
/** Host-local permission. It is never serialized into a channel event. */
