...
import { newMessageId, nowIso, WIRE_HEADERS } from '@agent-comm/core'
import { z } from 'zod'
...
export function createA2AMessage(input: CreateA2AMessageInput): A2AMessage {
  return {
    ...
    extensions: [...new Set([AGENTCOMM_A2A_EXTENSION_URI, ...(input.extensions ?? [])])],
...
export function readAgentCommRouting(message: A2AMessage): AgentCommRoutingMetadata | undefined {
  ...
  return {
    ...
    ...(typeof record.runtimeInstanceId === 'string' ? { runtimeInstanceId: record.runtimeInstanceId } : {}),
  }
}
...
export interface CreateAgentCommAgentCardInput {
  ...
  applicationExtensions?:
    | {
        uri: string
        version: string
        backwardCompatibleFrom?: string | undefined
        description?: string | undefined
      }[]
    | undefined
}
  ...
  const card: A2AAgentCard = {
    ...
    capabilities: {
      ...
      extensions: [
        ...
        ...(input.applicationExtensions ?? []).map((extension) => ({
          uri: extension.uri,
          description:
            extension.description ?? `AgentComm application extension ${extension.uri} ${extension.version}.`,
          required: false,
          params: {
            version: extension.version,
            ...(extension.backwardCompatibleFrom
              ? { backwardCompatibleFrom: extension.backwardCompatibleFrom }
              : {}),
          },
        })),
      ],
