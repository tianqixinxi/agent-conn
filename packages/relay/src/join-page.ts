    ...
    (function () {
      ...
      var copy = {
        ...
        ja: {
          ...
          openClaude: 'Claude Code で開く / インストール',
          openAgent: 'ローカル AgentComm Launcher で開く',
          coldStartHeading: '初回起動：',
          coldStartBody: 'Claude は永続プラグインのインストール許可を先に求めます。完了後に /reload-plugins を実行すると、AgentComm がチャンネルへの信頼を別途確認します。',
          manualInstall: 'Claude が自動インストールできない場合は、セッションで次を実行してください：',
        ...
        ko: {
          ...
          openClaude: 'Claude Code로 열기 / 설치',
          openAgent: '로컬 AgentComm Launcher로 열기',
          coldStartHeading: '최초 실행:',
          coldStartBody: 'Claude가 먼저 영구 플러그인 코드 설치 권한을 요청합니다. 설치 후 /reload-plugins를 실행하면 AgentComm이 채널 신뢰를 별도로 확인합니다.',
          manualInstall: 'Claude가 자동 설치를 실행하지 못하면 세션에서 다음 명령을 실행하세요:',
        ...
        es: {
          ...
          openClaude: 'Abrir / instalar con Claude Code',
          openAgent: 'Abrir con AgentComm Launcher local',
          coldStartHeading: 'Primer inicio:',
          coldStartBody: 'Claude primero solicita permiso para instalar código persistente del plugin. Después ejecuta /reload-plugins; AgentComm solicitará por separado la confianza del canal.',
          manualInstall: 'Si Claude no puede instalarlo automáticamente, ejecuta estos comandos en la sesión:',
        ...
        fr: {
          ...
          openClaude: 'Ouvrir / installer avec Claude Code',
          openAgent: 'Ouvrir avec AgentComm Launcher local',
          coldStartHeading: 'Premier démarrage :',
          coldStartBody: 'Claude demande d’abord l’autorisation d’installer le plugin persistant. Exécutez ensuite /reload-plugins ; AgentComm demandera séparément la confiance pour le canal.',
          manualInstall: 'Si Claude ne peut pas effectuer l’installation automatiquement, exécutez ces commandes dans la session :',
        ...
        de: {
          ...
          openClaude: 'Mit Claude Code öffnen / installieren',
          openAgent: 'Mit lokalem AgentComm Launcher öffnen',
          coldStartHeading: 'Erster Start:',
          coldStartBody: 'Claude bittet zuerst um Erlaubnis zur Installation des persistenten Plugins. Führen Sie danach /reload-plugins aus; AgentComm fragt das Kanalvertrauen separat ab.',
          manualInstall: 'Falls Claude die Installation nicht automatisch ausführen kann, führen Sie diese Befehle in der Sitzung aus:',
        ...
        pt: {
          ...
          openClaude: 'Abrir / instalar com Claude Code',
          openAgent: 'Abrir com o AgentComm Launcher local',
          coldStartHeading: 'Primeiro uso:',
          coldStartBody: 'Claude primeiro solicita permissão para instalar o plugin persistente. Depois execute /reload-plugins; o AgentComm solicitará separadamente a confiança no canal.',
          manualInstall: 'Se Claude não conseguir instalar automaticamente, execute estes comandos na sessão:',
        ...
        ru: {
          ...
          openClaude: 'Открыть / установить через Claude Code',
          openAgent: 'Открыть в локальном AgentComm Launcher',
          coldStartHeading: 'Первый запуск:',
          coldStartBody: 'Сначала Claude запросит разрешение на установку постоянного плагина. Затем выполните /reload-plugins; AgentComm отдельно запросит доверие к каналу.',
          manualInstall: 'Если Claude не может установить плагин автоматически, выполните в сессии следующие команды:',
      ...
      var promptCopy = {
        en: {
          ...
          install: 'Explain that a persistent plugin must be installed from https://github.com/tianqixinxi/agent-conn and ask exactly one yes/no question authorizing that plugin installation.',
          approved: 'Only after approval, run these commands:',
        ...
        zh: {
          ...
          install: '说明需要从 https://github.com/tianqixinxi/agent-conn 安装持久插件，并且只询问一次 yes/no 来授权安装该插件。',
          approved: '仅在我同意后运行以下命令：',
        ...
        ja: {
          ...
          install: 'https://github.com/tianqixinxi/agent-conn から永続プラグインをインストールする必要があると説明し、そのインストールを許可する yes/no 質問を一度だけ行ってください。',
          approved: '承認された後に限り、次のコマンドを実行してください：',
        ...
        ko: {
          ...
          install: 'https://github.com/tianqixinxi/agent-conn 에서 영구 플러그인을 설치해야 한다고 설명하고, 설치를 승인하는 yes/no 질문을 정확히 한 번만 하세요.',
          approved: '승인된 후에만 다음 명령을 실행하세요:',
        ...
        es: {
          ...
          install: 'Explica que debe instalarse un plugin persistente desde https://github.com/tianqixinxi/agent-conn y haz exactamente una pregunta sí/no para autorizar esa instalación.',
          approved: 'Solo después de la aprobación, ejecuta estos comandos:',
        ...
        fr: {
          ...
          install: 'Expliquez qu’un plugin persistant doit être installé depuis https://github.com/tianqixinxi/agent-conn et posez exactement une question oui/non autorisant cette installation.',
          approved: 'Uniquement après approbation, exécutez ces commandes :',
        ...
        de: {
          ...
          install: 'Erkläre, dass ein persistentes Plugin von https://github.com/tianqixinxi/agent-conn installiert werden muss, und stelle genau eine Ja/Nein-Frage zur Genehmigung dieser Installation.',
          approved: 'Führe erst nach der Genehmigung diese Befehle aus:',
        ...
        pt: {
          ...
          install: 'Explique que um plugin persistente deve ser instalado de https://github.com/tianqixinxi/agent-conn e faça exatamente uma pergunta sim/não autorizando essa instalação.',
          approved: 'Somente após a aprovação, execute estes comandos:',
        ...
        ru: {
          ...
          install: 'Объясните, что нужно установить постоянный плагин с https://github.com/tianqixinxi/agent-conn, и задайте ровно один вопрос да/нет для разрешения этой установки.',
          approved: 'Только после одобрения выполните следующие команды:',
