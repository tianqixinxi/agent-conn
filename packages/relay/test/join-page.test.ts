...
describe('relay: invitation and cold-start pages', () => {
  it('serves a multilingual terminal-first flow without a Claude deep link', async () => {
    ...
    expect(html).toContain('<header class="site-nav">')
    expect(html).toContain('class="quick-card"')
    expect(html).toContain('--cream: #fff8e8')
    expect(html).toContain('--pink: #f55da5')
    expect(html).toContain('--mint: #a9e8cc')
    expect(html).toContain('@media (max-width: 560px)')
    expect(html).toContain('window.navigator.clipboard.writeText(value).then(copied, fallback)')
    ...
    expect(html).toContain('Privaten Schlüssel nicht im Verlauf speichern')
